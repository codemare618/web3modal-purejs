const cuffies = (function(){
    ...
    // const cuffieAddress = "0xCc6de48BB57F301252B9964281B8624D187e1962";//testnet
    ...
    async function approveCheck(busd){
    ...
    async function buyMyCuffies(busd, params){
        const {provider, selectedAccount} = walletLib.getAccountInfo();
        ...
        return contractCall(cuffieContract.options.address, buyMethod, params);
    }

    async function approveBusd(busd, params) {
        // call buy
        const busdvalue = web3.utils.toWei(`${busd}`);
        const approveMethod = busdContract.methods.approve(cuffieAddress, busdvalue);
        console.log(busdContract.options.address);
        return contractCall(busdContract.options.address, approveMethod, params);
    }

    async function claimBoughtAmount(params){
        const method = cuffieContract.methods.claimBoughtAmount();
        return contractCall(cuffieContract.options.address, method, params);
    }

    async function contractCall(target, method, params) {
        // Finally send transaction
        // console.log(target);
        const {provider, selectedAccount} = walletLib.getAccountInfo();

        const {gaslimit, txHash, receipt, error} = params;
        ...
        let maximumGas = gaslimit;
        // const gas = await connectedWeb3.eth.estimateGas(txObject);
        /*
        try{
            maximumGas = await connectedWeb3.eth.estimateGas(txObject)
        }catch(error) {
            console.log(error);
        }*/
        connectedWeb3.eth.sendTransaction({...txObject, gas: maximumGas})
